...
const ResultPage = () => {
  ...
  // Ensure EvaluationType matches the actual structure of evaluation data
  type EvaluationType = {
    coursework_id: {
      file: string;
    };
    overall_score: number;
    criteriaA_score?: number;
    criteriaB_score?: number;
    criteriaC_score?: number;
  };
  ...
  useEffect(() => {
    if (evaluationData) {
      evaluationData.then((res: EvaluationType[]) => setEvaluation(res));
    }
  }, [evaluationData]);
  ...
  const getFile = (fileId: string) => {
    ...
    const file_obj = fileData.find((res: any) => res.uploadDate === fileId);
    if (file_obj) {
      setFileObj(file_obj);
      getFileURL(file_obj);
    }
    ...
    <div className="w-[100%] h-[80%] flex">
      <div className="w-fit h[100%] mt-5">
        <div className="flex bg-[#f5f5f5] p-3 rounded-b-none rounded-2xl items-center justify-between">
          {fileObj && (
            <div className="text-sm font-semibold bg-white px-2 py-1 rounded-full">
              {fileObj.name}
            </div>
          )}
        </div>
        {fileURL && (
          <div className="bg-white pb-1 rounded-t-none rounded-xl pl-1">
      ...
      <div className="ml-3 mt-9 w-[28rem]">
        <div className="flex bg-white p-3 rounded-3xl justify-between items-center">
          <div className="ml-2">
            ...
            <h4 className="text-[20px] text-[#7A8196] font-bold">
              Remark : <span className="text-[#3CC28A]">Good</span>
            </h4>
            <p className="text-[12px] text-[#98A1BB]">Evaluated on 23 Jul 2024</p>
          </div>
          <div>
            {currentEvaluation && currentEvaluation.overall_score && (
              <CircularProgressRing
                score={currentEvaluation.overall_score}
                maxScore={maxScore}
                size={80}
                strokeWidth={10}
              />
            ...
            <AccordionItem value="section-1">
              <AccordionTrigger className="p-4 bg-white">
                <div className="flex items-center justify-between">
                  {currentEvaluation && currentEvaluation.criteriaA_score && (
                    <CircularProgressRing
                      score={currentEvaluation.criteriaA_score}
                      maxScore={maxScore}
                      size={80}
                      strokeWidth={10}
                    />
              ...
              <AccordionContent className="p-4 bg-white border-white rounded-lg">
                <div>
                  ...
                  <div className="border border-[#3CC28AB8] p-1 rounded-xl bg-[#d2f8e8b8]">
                    <p className="flex text-[12px] text-[#3D404B] items-start gap-2">
                      <Image src={Right} alt="img" className="mt-2" /> Demonstrates a good
                      understanding of the prescribed title and the associated knowledge questions.
                    </p>
                  </div>
                  ...
                  <div className="border border-[#f3bb2d05]-100 p-1 rounded-xl bg-[#f7d37705]">
                    <p className="flex text-[12px] text-[#3D404B] items-start gap-2">
                      <Image src={Info} alt="img" className="mt-2" /> Demonstrates a good
                      understanding of the prescribed title and the associated knowledge questions.
                    </p>
                  </div>
                </div>
